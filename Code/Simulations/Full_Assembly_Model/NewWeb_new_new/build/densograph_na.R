#$Id: densograph_na.R 849 2007-01-19 07:45:45Z cvsrep $
library(KernSmooth)
library(lattice)
source("/home/axel/NewWeb/src/my.bkde3D.R")

bandwidth=1
range=list(c(-13.5,3),c(-4,2))
gridsize=c(201,201)
#gridsize=c(51,51)
#gridsize=c(15,15)
...
grid <- expand.grid(x=estL1$x1, y=estL1$x2,kinds=c("producers") )
...
#     panel.lines(c(-14,-14),y=c(-10,0));
 ...
 cp<-contourplot(z~x*y|kinds, grid,
	...
	ylab=
	expression(paste(log[10]," ",B/A," ",group("[",kg/m^2,"]"))),
        layout=c(1,1),
        aspect="iso",
        as.table=TRUE,
        panel = pf,
...
postscript(file="densograph_na.ps", horizontal = FALSE,
