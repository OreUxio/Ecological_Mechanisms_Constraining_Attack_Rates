// $id: SizeSpectrum.cc$
#include <string>
const std::string version("$Revision: 2402 $");
...
const int BITMAPWIDTH=1200, BITMAPHEIGHT=300;
...
cmplx kappa0Hat_being_eaten(cmplx zeta)
{
  if(balance_case==1){
    ...
    return 
      -
      search_gamma*h/
      (search_gamma*phiTilde+h)*
      conj(sHat(conj(zeta)))
      +
      search_gamma*search_gamma*NTilde*h/
      (search_gamma*phiTilde+h)/
      (search_gamma*phiTilde+h)*
      sHat(zeta-cmplx(0,3-lambda-n))*
      conj(sHat(conj(zeta)));
...
{ 
  return double(3)*pow(eta4,cmplx(0,-4)*zeta)/
    (double(3) + cmplx(0,-25)*zeta - 
     double(70)*(zeta*zeta) + 
     cmplx(0,80)*
     (zeta*zeta*zeta) + 
     double(32)*(zeta*zeta*(zeta*zeta))
     );
}
...
cmplx exclusionHat(cmplx z){
  ...
  if(wD != 0){
    return 
      (exp(-(wD*z)*(wD*z)/double(2))-double(1))*(2*D/(wD*wD));
  }else{
    return -D*z*z;
  }
}
...
vector<double> poles(){
  vector<double> p;
  if(!noSpecies){
    for(double i=1;i>(1-n)/2;i-=(1-n)){
      p.push_back(-i - ( communitySpectrum ? 1-n : 0 ) );
      if(not (balance_case == 1 && i < 3*(1-n)/2)){
...
void SizeSpectrum::initialize_dynamics(){
  ...
  if(!phyto_spectrum){
    _min_u = -1e-9 + log(1e-7) - log(beta);// -_N*_delta_u/3;
  ...
  for(int i=_N;i-->0;){
    ...
    if(!phyto_spectrum){
      double ex=exp(-0.6*(u(i)-lower_cutoff));
      damper[i]=-8*ex/(ex+1);
    }else{
  ...
  for(int i=_N+1;i-->0;){
    normalizing_kernel[i]=kernel_function(xi(i));
    normalizing_kernel[i]*=(1/double(2*_N));
...
int SizeSpectrum::dynamics(ODE_vector const & state, 
  ...
  if( nonlinear_version <= 2 ){
    ...
    //to_xmgr(pl,&re_work[0],0);FATAL_ERROR("check tmp.dat!");
    for(int i=_N;i-->0;){
      // ...dynamics scaled by tildeBtot for historical reasons.
      re_work[i] = re_work[i]*equilibriumB[i]/(mn1[i]*tildeBtot) + X[i];
    }
    if(phyto_spectrum){// overwrite result for the phytoplankton part
      ...
      for(int i=phyto_imbalance.size();i-->0;){
	re_work[i]=
	  -phyto_damping_rate * (phyto_imbalance[i]/equilibriumB[i]+1-phyto_boost)*(1/phyto_boost) * (1/tildeBtot) // logistic growth
	  + (_work_mu.re(i)+muTilde)*(1/tildeBtot);
...
void SizeSpectrum::MERP_report_size_classes(ostream & os){
  ...
  for(int i=0;i<n_size_classes;i++){
    os << (i+1) << " " 
       << pow(eta4,4)*exp(u(index_of(min_u()+M_LN10*(i+1)))) << endl;
  }
}
...
void TheoreticalSizeSpectrum::comprehensive_zero_search(){
  ...
  for(int ii=0;ii<zeros_found;ii++){
    ...
    cmplx p1=zero[i],p2=zero[i]+0.15*v[i]/cmplx(0,abs(v[i]));
    cout << "@    line " 
	 << p1.real() << ", " 
	 << p1.imag() << ", " 
	 << p2.real() << ", " 
...
void read_configuration(int argc,char** argv){
  int c;
  const char * formatstring = "vg:W:M:";
...
#define VAR lambda
  for(VAR=1.5;VAR<2.5;VAR*=1.005){
    set_dependent_constants();
    TheoreticalSizeSpectrum b_theo(-10,10);
    timespec t = {0,0.2 * 1e9};
    nanosleep(&t,&t);
    b_theo.plot_zeros(plotter,VAR);
  }
#undef VAR
  exit(0);
}
...
int main(int argc,char** argv){
  ...
  nanosleep(&tsleep,&trem);
  myPlotter<> tplotter(!communitySpectrum);
  nanosleep(&tsleep,&trem);
  ...
  SizeSpectrum b(256,log(pow(10,0.1)));
	...
	if(t==0 || (gif_image_file && t > next_gif_image_t)){
	  ...
	  if(MERP_output_file){
	    ...
	    b.MERP_write_state(MERP_os);
	    {
	      ...
	      b.to_xmgr(frame,&b.individual_spectrum()[0],0);
	      frame_number++;
	    }
	  }
	}
