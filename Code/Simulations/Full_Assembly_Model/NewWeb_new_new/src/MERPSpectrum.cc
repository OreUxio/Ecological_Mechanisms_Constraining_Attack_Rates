// 	$Id: MERPSpectrum.cc 2517 2017-12-12 17:08:31Z axel $	
#include <string>
const std::string version("$Revision: 2517 $");
const std::string source("$Source$ $Date: 2017-12-12 17:08:31 +0000 (Tue, 12 Dec 2017) $");
...
// DEAD CODE
// This would be required only to compute beta with x0 > 0 and n != 3/4 
...
double mesh_size=0;
double mesh_size_upper=HUGE_VAL;
double absoluteF=0;
double lambda=2+q-n;
double relativeF=0;
double invasion_pressure=0;
double rho=1;
double wD=w;
double bRange=2;  
double x=1;//Scalefactor for transition to eutrophication
int max_zeros=0;
int doSimulate=true;
...
static cfgStruct cfg[] = 
  { 
    ...
    CFGDOUBLE(sigma),
    CFGDOUBLE(mesh_size),
    CFGDOUBLE(mesh_size_upper),
    CFGDOUBLE(absoluteF),
    CFGDOUBLE(relativeF),
    CFGDOUBLE(invasion_pressure),
    CFGDOUBLE(lambda),
    CFGDOUBLE(rho),
    CFGDOUBLE(x),
    CFGDOUBLE(wD),
...
class myPlotter : public ofstream, public PLOTTER, public somePlotter {
  bool _active;
...
public:
  ...
  myPlotter(bool active, FILE * outfile);
  myPlotter(bool active, ostream & os);
  ~myPlotter();
...
myPlotter< PLOTTER >::myPlotter(const char * filename): 
  _active(filename),
  ofstream(filename),
  //PLOTTER((ostream &)*this) // for some reason, only last frame is saved!!
  PLOTTER(stderr) // using stderr, all frames are going out.
{
  if(_active){
    if (PLOTTER::openpl () < 0)          // open Plotter
      FATAL_ERROR("could not open plotter");
...
template< class PLOTTER >
myPlotter< PLOTTER >::myPlotter(bool active, FILE * outfile): 
   _active(active),
   _outfile(outfile),
   PLOTTER(stdin,outfile,stderr) 
   //PLOTTER(stdin,stderr,stdout) 
 {
   if(_active){
     if(outfile == NULL){
       FATAL_ERROR("outfile not open");
     }
     if (PLOTTER::openpl () < 0)          // open Plotter
       FATAL_ERROR("could not open plotter");
   }
...
template< class PLOTTER >
myPlotter< PLOTTER >::myPlotter(bool active, std::ostream & os): 
   _active(active),
   _outfile(NULL),   
   PLOTTER(os) 
 {
...
myPlotter< PLOTTER >::~myPlotter()
{
  if(_active){
    if (PLOTTER::closepl () < 0)          // close Plotter
      FATAL_ERROR("could not close plotter");
    if(_outfile){
      fclose(_outfile);
    };
  }
  ...
  if(mesh_size){
    PLOTTER::
      fline(log(mesh_size)/M_LN10,-_nominalHeight,
	    log(mesh_size)/M_LN10,+_nominalHeight/2.1);
    if(mesh_size_upper < exp(upper_cutoff)){
      PLOTTER::
	fline(log(mesh_size)/M_LN10,+_nominalHeight/2.1,
	      log(mesh_size_upper)/M_LN10,+_nominalHeight/2.1 );
      PLOTTER::
	fline(log(mesh_size_upper)/M_LN10,+_nominalHeight/2.1,
	      log(mesh_size_upper)/M_LN10,-_nominalHeight);
    }
    PLOTTER::endpath();
  }else{
    PLOTTER::
...
class SizeSpectrum : public ODE_dynamical_object
{
  ...
  double _delta_xi;
  double _min_u;
  bool _averaging;  // shall we do time averaging?
  vector<double> _integral; // for averaging
  double _averaging_start_time;
...
public:
  ...
  const vector <double> get_fishing() const {return fishing;}
  void set_fishing(const vector<double> fi){fishing=fi;}
private:
  void initialize();
public:
...
void SizeSpectrum::read_state_from(const ODE_vector & state){
  ...
  if(_averaging){
    for(int i=_N;i-->0;){
      _integral[i]=state[i + _N];
    }
  }
}  

void SizeSpectrum::start_averaging(){
  _averaging = true;
  _integral.resize(_N);
  for(int i=_N;i-->0;){
    _integral[i]=0;
  }
  _averaging_start_time = current_time;
}

cmplx betaHat(cmplx zeta); // see below
void SizeSpectrum::save_average(){
  //	re_work[i] = (exp(state[i]/equilibriumB[i])-1)*equilibriumB[i];
  double T = current_time - _averaging_start_time;
  vector<double> c(_N);
  const double betafac = betaHat(cmplx(0,lambda-2)).real();
  for(int i=_N;i-->0;){
    c[i]=(_integral[i]/T + betafac * equilibriumB[i])*(gamma_boost/NTilde);
  }
  
  ofstream average("average_spectrum.dat");
  for(int i=0;i<N();++i){
    if(c[i] > 1e-20)
      average << u(i)/M_LN10 << " " << log10(c[i]) << endl;
    else
      average << u(i)/M_LN10 << " " << -20 << endl;
  }
}
...
inline cmplx kappa0Hat_being_eaten_direct(cmplx zeta){
  return
    -x*search_gamma*h/
...
inline cmplx kappa0Hat_being_eaten_release(cmplx zeta){
  return
...
cmplx kappa0Hat_being_eaten(cmplx zeta)
{
  if(balance_case==1){
    ...
    return 
      kappa0Hat_being_eaten_direct(zeta) +
      kappa0Hat_being_eaten_release(zeta);
  }
}
...
void SizeSpectrum::initialize_dynamics(){
  ...
  for(int i=_N;i-->0;){
    ...
    if(mesh_size){
      fishing[i]=
	(u(i)>log(mesh_size) && u(i) < log(mesh_size_upper) ?
	 -absoluteF-exp_minus_u_over_4[i]*relativeF : 0);
    ...
    if(!phyto_spectrum){
      double ex=exp(-10*(u(i)-lower_cutoff));
      damper[i]=-8*ex/(ex+1);
    }else{
...
int SizeSpectrum::dynamics(ODE_vector const & state, 
  ...
  if( nonlinear_version <= 2 ){
    ...
    ////// Print food concentrations relative to half-saturation:
    if(false){
      cout <<  1/(1-f[index_of(log(1e-6))]) << "\t"
	   <<  1/(1-f[index_of(log(1e-3))]) << "\t"
	   << 1/(1-f[index_of(log(1e-0))]) << "                   \n";
    }
  ...
  if(invasion_pressure>0 && nonlinear_version >= 2){
    for(int i=2*_N;i-->0;){
      int ii=( i<3*_N/2 ? i :  i-2*_N);
      if(ii<0){
	//
	...
	time_derivative[i] +=
	  invasion_pressure * exp_minus_u_over_4[i] *
	  exp(-state[i]/equilibriumB[i]);
      }
    }
  }
...
void SizeSpectrum::save_state(const char * filename) const{
  ...
  if(gamma_as_pp_boost){
    for(int i=0;i<_N;i++){
      os << u(i) << " " << _b[i] + log(gamma_boost)*equilibriumB[i] << endl;
    }
  }else{
    for(int i=0;i<_N;i++){
      os << u(i) << " " << _b[i] << endl;
...
vector<double> SizeSpectrum::individual_spectrum(){
  ...
  for(int i=_N+1;i-->0;){
    _work[i]*=normalizing_beta_hat[i];
    //    _work[i]*=1-exp(-0.2*(_N-40-i))/(1+exp(-0.2*(_N-40-i))); // anti-aliasing
  }
  _work.fft_backward();
...
public:
  ...
  template< class PLOTTER >
  void plot_zeros(PLOTTER & p,double value=0);
  ...
  void save_fancy_zero_pnm(const char *filename);
};
...
// Here's a C implementation of hsv2rgb based on Agoston's Computer
// Graphics and Geometric Modeling: Implementation and Algorithms
// p. 304, with H ∈ [0, 360] and S,V ∈ [0, 1].
typedef struct {
    double r;       // ∈ [0, 1]
    double g;       // ∈ [0, 1]
    double b;       // ∈ [0, 1]
...
void TheoreticalSizeSpectrum::save_fancy_zero_pnm(const char * filename){
  const int pixels=800;
  const int maxval=255;
  const double range=4*0.5;
  const int n_arg_classes=8;
  ...
  for(int j=0; j<pixels; j++){
    for(int i=0; i<pixels; i++){
      const cmplx xi=
	cmplx(range*(2*double(i)/pixels-1),-range*(2*double(j)/pixels-1));
      cmplx kernel_fun = kernel_function(xi);
      double kernel_arg = arg(kernel_fun);
      unsigned char pixel[] = {0,0,0};
      kernel_arg = 2*kernel_arg/(2*M_PI)+4;
      kernel_arg = kernel_arg - int(kernel_arg);
      //kernel_arg = int(n_arg_classes*kernel_arg)/double(n_arg_classes);
      double v = pow(cos(2*M_PI*kernel_arg),100);
#if 0
      const hsv hsv_color={kernel_arg*360,1,v};
      //const hsv hsv_color={double(i)/pixels*360,1,1}; // test
      const rgb rgb_color=hsv2rgb(hsv_color);
      const char r=maxval*rgb_color.r;
      const char g=maxval*rgb_color.g;
      const char b=maxval*rgb_color.b;
      pixels = {r,g,b};
#else
      cmplx common_factor =
	tildeBtot*betaHat(xi)*
	conj(betaHat(conj(xi+cmplx(0,(3-lambda-n)))));
      
      valarray<cmplx> component(4);
      valarray<double> phase_line(4);
      cmplx zeta = xi+cmplx(0,3-lambda-n);
      
      component[0] = kappa0Hat_eating(zeta) * common_factor;
      component[1] = kappa0Hat_being_eaten_direct(zeta) * common_factor;
      component[2] = kappa0Hat_being_eaten_release(zeta) * common_factor;
      component[3] = exclusionHat(xi);

      //double component_color[][3]={{1,0,0},{0,1,0},{0,0,1},{0.5,0.5,0.5}};
      double component_color[][3]={{0,1,0},{1,0,0},{0,0,1},{0.7,0.7,0.7}};

      // REPORT((component[0]+component[1]+component[2]+component[3])/
      // 	     kernel_function(xi) );
      valarray<cmplx> off = component/kernel_fun-1;
      // cout << off[0] << " ";
      // cout << off[1] << " ";
      // cout << off[2] << " ";
      // cout << off[3] << endl;
      double colour_mix[3] = {0,0,0};
      for(int comp=4;comp-->0;){
	double a = arg(component[comp]);
	a = a/(2*M_PI)+4;
	a = a - int(a);
      //kernel_arg = int(n_arg_classes*kernel_arg)/double(n_arg_classes);
	phase_line[comp] = pow(cos(2*M_PI*a),100);
      }
      for(int col=3;col-->0;){
	for(int comp=4;comp-->0;){
	  colour_mix[col] +=
	    (1-phase_line[comp])*component_color[comp][col]*
	    exp(-7*pow(abs(off[comp]),3));
	}
	pixel[col] = maxval*min(1.0,colour_mix[col]);
      }
#endif
...
void TheoreticalSizeSpectrum::comprehensive_zero_search(){
  ...
  for(int ii=0;ii<zeros_found;ii++){
    ...
    cmplx p1=zero[i],p2=zero[i]+0.2*v[i]/cmplx(0,abs(v[i]));

    const char * graph_number="z";
    
    if(abs(p1.real())<=2 && abs(p1.imag())<=2){
      arrows << "@with line" << endl;
      arrows << "@    line on" << endl;
      arrows << "@    line loctype world" << endl;
      arrows << "@    line g" << graph_number << endl;
      arrows << "@    line " 
	   << p1.real() << ", " 
	   << p1.imag() << ", " 
	   << p2.real() << ", " 
...
template< class PLOTTER >
void TheoreticalSizeSpectrum::plot_zeros(PLOTTER & p,double value){
...
void read_configuration(int argc,char** argv){
  int c;
  const char * formatstring = "vg:W:M:Z:R:f:G:qk:F:";
  ...
  while(argc>1){
	...
	break;
      case 'F':
	fancy_zero_pnm=optarg;
	break;
      case 'f':
	frame_rate=atof(optarg);
	break;
      case 'G':
	gif_rate=atof(optarg);
	break;
      case 'R':
	nutrient_ramp_rate=atof(optarg);
	break;
      case 'W':
	output_file=optarg;
	break;
...
int main(int argc,char** argv){
  ...
  ostringstream GIF_DELAY;
  GIF_DELAY << ceil(100/frame_rate);
  Plotter::parampl ("GIF_DELAY", const_cast<char*>(GIF_DELAY.str().c_str()));

  timespec tsleep = {0,0.1 * 1e9},trem;
  myPlotter<> plotter(verbose);
  nanosleep(&tsleep,&trem);
  myPlotter<> tplotter(false);
  nanosleep(&tsleep,&trem);
  myPlotter<> iplotter(verbose && !communitySpectrum && !noSpecies);
  //myPlotter<GIFPlotter> gif_iplotter(gif_image_file);
  myPlotter<GIFPlotter> gif_iplotter((char *)0);
  Plotter::parampl ("PAGESIZE", const_cast<char*>("letter"));
  Plotter::parampl ("USE_DOUBLE_BUFFERING",const_cast<char*>("yes"));
  Plotter::parampl ("BITMAPSIZE",const_cast<char*>("350x350"));
  nanosleep(&tsleep,&trem);
  myPlotter<> cplotter(verbose);
  ...
  if(fancy_zero_pnm){
    b_theo.save_fancy_zero_pnm(fancy_zero_pnm);
  }
  ...
  int plot_skip=(verbose ? 100 : 1000);
  int plot_step=0;
  ...
  double t_target=t_final; // we may choose to target earlier times later
  while(t<t_final && not exit_now){
    {
      ODE_state state(&b);
      while(t<t_final && not exit_now){	
	if(!doSimulate){}
	else{
	  static int next_stage_time=10;
	  if(t > next_stage_time){
	    static int stage=0;
	    stage++;
	    next_stage_time*=10;
	    double max_step_size=sqrt(next_stage_time/1e5);
	    if(nutrient_ramp_rate)
	      max_step_size=
		min(gif_rate/frame_rate,max_step_size);
	    ostringstream MAX_STEP_SIZE;
	    MAX_STEP_SIZE << max_step_size;
	    set_cfg_parameter("MAX_STEP_SIZE",MAX_STEP_SIZE.str().c_str());
	    break;
	  }
	}

	if(pulse_perturbation && t>pulse_perturbation*time_unit){
	  b.no_more_fishing();
	  //	  break;
	}
	...
	while(gif_image_file && t >= next_gif_image_t){
	  b.read_state_from(state);
	  b.plot_individuals(gif_iplotter); 
	  if(nutrient_ramp_rate){
	    next_gif_image_t+=gif_rate/frame_rate;
	    if(t >= /*20*/1 && gamma_boost < 3){
	      gamma_boost+=nutrient_ramp_rate*(gif_rate/frame_rate);
	      b.initialize_dynamics();
	    }
	    t_target=next_gif_image_t;
	  }else{
#if 0 // Fig2c-anim.gif
	    static vector<double> final_fishing=b.get_fishing();
	    vector<double> new_fishing(final_fishing);
	    for(int i=b.N();i-->0;) new_fishing[i]*=min(double(1),t/10);
	    b.set_fishing(new_fishing);
#endif

	    next_gif_image_t+=gif_rate/frame_rate;
	    REPORT(next_gif_image_t);
	    //	    next_gif_image_t*=1.1;
	  }
	  // ... in addition, biomass in size classes into .cvs file
	  if(MERP_output_file){
	    static ofstream MERP_os(MERP_output_file);
	    b.MERP_write_state(MERP_os);
	    // ... and also write .dat movie frames.
	    {
		...
		frame << exp(b.u(i))*1e-3 << " " << exp(c[i]) << endl;
	      ...
	      for(int i=0;i<b.N();++i){
		if(exp(c[i]) > 1e-20)
		  frame << b.u(i)/M_LN10 << " " << c[i]/M_LN10 << endl;
		else
		  frame << b.u(i)/M_LN10 << " " << -20 << endl;
	      }
#endif
	      frame_number++;
	    }
	...
	if(doSimulate){
	  if(state.integrate_one_step(t_target)){
	    state.diagnosis();
	    break;
	  }
	  step_counter++;
  ...
  ODE_vector state(b.N());
  ODE_vector time_derivative(b.N());
  ...
  {
    ODE_state state(&b);
    state.integrate_until(b.current_time+10);
  }
  b.save_average();  
#endif
  ...
  for(int sample=0;sample < 100;sample++){
    ostringstream filename;
    filename  << "B_" << setfill('0') << setw(2) 
	      << sample+1 << "_final.dat";
    REPORT(sample);
    ofstream sample_file(filename.str().c_str());
    vector<double> c=b.individual_spectrum();
    for(int i=0;i<b.N();++i){
      if(exp(c[i]) > 1e-20)
	sample_file << b.u(i)/M_LN10 << " " << c[i]/M_LN10 << endl;
      else
	sample_file << b.u(i)/M_LN10 << " " << -20 << endl;
    }
    b.plot_individuals(iplotter);
    {
      ODE_state state(&b);
      state.integrate_until(b.current_time+2);
    }
  }
#endif
  exit(0);
