...
//~ #endif // experimental code to make movies
  ...
  if(print || pgm_file){
	...
	//~ out2 = out2 + str + ".dat";
	string out3 = "after_interaction_matrix.dat";
	//~ out3 = out3 + str + ".dat";
	string out4 = "after_agg_for_matrix.dat";
	//~ out4 = out4 + str + ".dat";
	string out5 = "during_interaction_matrix.dat";
	//~ out5 = out5 + str + ".dat";
	string out6 = "during_agg_for_matrix.dat";
	//~ out6 = out6 + str + ".dat";
	string out7 = "after_interaction_matrix1.dat";
	//~ out7 = out7 + str + ".dat";
	string out8 = "after_agg_for_matrix1.dat";
	//~ out8 = out8 + str + ".dat";
	string out9 = "during_interaction_matrix1.dat";
	//~ out9 = out9 + str + ".dat";
	string out10 = "during_agg_for_matrix1.dat";
	//~ out10 = out10 + str + ".dat";
	before_interaction_matrix.open(out1.c_str(),ios_base::app);
	before_agg_for_matrix.open(out2.c_str(),ios_base::app);
	during_interaction_matrix.open(out5.c_str(),ios_base::app);
	during_agg_for_matrix.open(out6.c_str(),ios_base::app);
	//~ after_interaction_matrix.open(out3.c_str(),ios_base::app);
	//~ after_agg_for_matrix.open(out4.c_str(),ios_base::app);
	//~ after_interaction_matrix1.open(out7.c_str(),ios_base::app);
	//~ after_agg_for_matrix1.open(out8.c_str(),ios_base::app);
	during_interaction_matrix1.open(out9.c_str(),ios_base::app);
	during_agg_for_matrix1.open(out10.c_str(),ios_base::app);
		...
		//~ }else{
			double fitness = 0;
				while(fitness<=0){
				new_species = web.speciate(NewSpecies::animal);
				new_species->set_aggressivity_g(agg);
				web.initialize_for_integration();
				web.get_top_down_strength();
				fitness = web.invasion_fitness(new_species);
				cout << "invader fitness = " << fitness << endl;
			}
		//~ }
		// new_species->set_aggressivity_g(agg*new_species->aggressivity_g()/new_species->attack_rate_a());
		...
		//~ Print out original community matrix + first invader (not relaxed)
		for(int i=web.s.size();i-->0;){
			during_agg_for_matrix << web.s(i).unique_id() << " "<<web.s(i).aggressivity_g() << " " << web.s(i).attack_rate_a() << " " << web.s(i).biomass_abundance_B() << endl;
			for(int j=web.s.size();j-->0;){
		...
		//~ Print out original community matrix + first invader (relaxed)
		for(int i=web.s.size();i-->0;){
			during_agg_for_matrix1 << web.s(i).unique_id() << " "<<web.s(i).aggressivity_g() << " " << web.s(i).attack_rate_a() << " " << web.s(i).biomass_abundance_B() << endl;
			//~ if(web.s(i).aggressivity_g()==agg){
				//~ fitness = 1;
			//~ }
			for(int j=web.s.size();j-->0;){
				if(j!=0){during_interaction_matrix1 << web.precomputed[i].c[j] << ",";}else{during_interaction_matrix1 << web.precomputed[i].c[j] << endl;}
			}
		}
