/* Modified by Axel Rossberg */
/**************************************************************************/
/*                                                                        */
/*  parsecfg - a library for parsing a configuration file                 */
/*  Copyright (C) 1999-2001 Yuuki NINOMIYA <gm@debian.or.jp>              */
/*                                                                        */
/*  patches to use the CLHEP evaluator                                    */
/*  Copyright (C) 2007      Axel G. Rossberg <axel@rossberg.net>          */
...
/* $Id: parsecfg.c 1953 2010-11-07 01:35:41Z axel $ */
...
static cfgErrorCode parse_simple(const char *file, FILE *fp, char *ptr, cfgStruct cfg[], int *line);
static cfgErrorCode parse_values_between_braces(const char *file, FILE *fp, const char *parameter, cfgStruct cfg[], int *line, cfgFileType type, int section, const char *parameter_buf, int parameter_line);
static char *parse_word(char *ptr, char **word, cfgKeywordValue word_type);
static cfgErrorCode store_value(cfgStruct cfg[], const char *parameter, const char *value, cfgFileType type, int section);
static cfgErrorCode parse_ini(const char *file, FILE *fp, char *ptr, cfgStruct cfg[], int *line, int *section);
static cfgErrorCode alloc_for_new_section(cfgStruct cfg[], int *section);
...
static void cfgFatalFunc(cfgErrorCode error_code, const char *file, int line, const char *str)
{
	...
	case CFG_CREATE_FAIL:
		fprintf(stderr, ("Cannot create configuration file `%s'.\n"), file);
		break;
...
static cfgErrorCode parse_simple(const char *file, FILE *fp, char *ptr, cfgStruct cfg[], int *line)
{
	...
	/* Patch by A.R.: Use exclamation mark to declare new helper parameters*/
	int declared_on_the_fly=0;
	if (*ptr == '!') {
	  declared_on_the_fly=1;
	...
	if (*ptr == '{') {
		...
		/* patch inserted here by A.R. */
		if(evaluate_expression(&value)){
 		  free(parameter);
 		  return (CFG_SYNTAX_ERROR);
		}
		if(declared_on_the_fly){
		  char *endptr;
		  double tmp = strtod(value, &endptr);
		  set_evaluator_variable(parameter,tmp);
		}else
		/* end of patch */
...
static char *parse_word(char *ptr, char **word, cfgKeywordValue word_type)
{
	...
	for (;;) {
		if (quote_flag == CFG_NO_QUOTE) {
		  /*patched by A.R.*/
			if ((word_type != CFG_VALUE && *(ptr + len) == ' ') || *(ptr + len) == '\t' || *(ptr + len) == '\0' || *(ptr + len) == '#' || (*(ptr + len) == '=' && word_type == CFG_PARAMETER) || (*(ptr + len) == ']' && word_type == CFG_SECTION)) {
			  /*
			...
			case CFG_INT:
				...
				if (type == CFG_INI) {
					...
					*(int *) (cfg[num].value) = tmp;
					/* patch A.R.: */
					set_evaluator_variable(cfg[num].parameterName,tmp);
				}
				return (CFG_NO_ERROR);
			...
			case CFG_UINT:
				...
				if (type == CFG_INI) {
					...
					*(unsigned int *) (cfg[num].value) = utmp;
					/* patch A.R.: */
					set_evaluator_variable(cfg[num].parameterName,utmp);
				}
				return (CFG_NO_ERROR);
			...
			case CFG_FLOAT:
				...
				if (type == CFG_INI) {
					...
					*(float *) (cfg[num].value) = ftmp;
					/* patch A.R.: */
					set_evaluator_variable(cfg[num].parameterName,ftmp);
				}
				return (CFG_NO_ERROR);
			...
			case CFG_DOUBLE:
				...
				if (type == CFG_INI) {
					...
					*(double *) (cfg[num].value) = dtmp;
					/* patch A.R.: */
					set_evaluator_variable(cfg[num].parameterName,dtmp);
				}
				return (CFG_NO_ERROR);
