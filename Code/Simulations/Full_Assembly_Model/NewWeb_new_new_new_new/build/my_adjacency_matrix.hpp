...
namespace boost {
  ...
  public:
    ...
    adjacency_matrix(vertices_size_type n_vertices) 
      : m_matrix(m_matrix_size(n_vertices)),
    ...
    // Number of elements required for internal storage
    vertex_descriptor m_matrix_size(vertices_size_type n_vertices){
      return (Directed::is_directed ? 
  ...
  // O(1) for undirected graphs or graphs without edges, otherwise
  // O(V^2).  To make this O(1) also in the latter case, one would
  // have to choose another representation of the adjacency matrix in
  // memory.
  template <typename D, typename VP, typename EP, typename GP, typename A>
  inline typename adjacency_matrix<D,VP,EP,GP,A>::vertex_descriptor
  add_vertex(const VP& vp, adjacency_matrix<D,VP,EP,GP,A>& g) 
  {
    ...
    size_type new_size=g.m_vertex_set.size()+1;
    g.m_vertex_set=typename Graph::VertexList(0, new_size);
    g.m_matrix.resize(g.m_matrix_size(new_size));
    g.m_vertex_properties.push_back(vp);

    if(D::is_directed && g.m_num_edges>0){
      size_type u,v;
      for(u=new_size-1;u-->0;){
	detail::set_edge_exists(g.m_matrix[u * new_size + (new_size-1)], 
				false, 
				0 );
	for(v=new_size-1;v-->0;){
	  g.m_matrix[u * new_size + v]=
	    g.m_matrix[u * (new_size-1) + v];
	}
      }
    }

    return *(g.m_vertex_set.end()-1);
  }

  // O(V^2)
  template <typename D, typename VP, typename EP, typename GP, typename A>
  inline void
  remove_vertex(typename adjacency_matrix<D,VP,EP,GP,A>::vertex_descriptor ux,
                adjacency_matrix<D,VP,EP,GP,A>& g)
  {
    typedef adjacency_matrix<D,VP,EP,GP,A> Graph;
    typedef typename Graph::vertices_size_type size_type;
    ...
    if( D::is_directed ){
      g.m_num_edges-=out_degree(ux,g);
      g.m_num_edges-=in_degree(ux,g);
      g.m_num_edges+=detail::get_edge_exists(g.get_edge(ux,ux), 0);

      size_type u,v;
      for(u=0; u<new_size ; u++){
	for(v=0; v<new_size; v++){
	  g.m_matrix[u * new_size + v]=
	    g.m_matrix[(u+(u>=ux)) * (new_size+1) + v+(v>=ux)];
	}
      }
    }else{
      g.m_num_edges-=out_degree(ux,g);
      ...
      for(u=0; u<new_size ; u++){
	for(v=0; v<=u; v++){
	  g.m_matrix[u * (u + 1)/2 + v]=
	    g.m_matrix[(u + (u>=ux)) * (u + (u>=ux) + 1)/2 + v + (v>=ux)];
	}
      }
    }
    g.m_vertex_set=typename Graph::VertexList(0, new_size);
    g.m_matrix.resize(g.m_matrix_size(new_size));
    g.m_vertex_properties.erase(g.m_vertex_properties.begin()+ux);
  }
