/**** WARNING: USING THIS SEEMS TO GIVE SLIGHTLY DIFFERENT RESULTS
      WITH EACH RUN!! ****/
...
#ifndef BLAS_H
int idamax_ (const int  *n, double *dx, const int  *incx);
double dasum_ (const int  *n, double *dx, const int  *incx);
int daxpy_ (const int  *n, double  *da, double *dx, const int  *incx, double *dy, const int  *incy);
int dcopy_ (const int  *n, double *dx, const int  *incx, double *dy, const int  *incy);
double ddot_ (const int  *n, double *dx, const int  *incx, double *dy, const int  *incy);
double dnrm2(const int  *n, double *dx, const int  *incx);
int dscal_ (const int  *n, double  *da, double *dx, const int  *incx);
int dswap_ (const int  *n, double *dx, const int  *incx, double *dy, const int  *incy);
int drot_ (const int  *n, double *dx, const int  *incx, double *dy, const int  *incy, double  *c, double  *s);
int isamax_ (const int  *n, float *dx, const int  *incx);
float sasum_ (const int  *n, float *dx, const int  *incx);
int saxpy_ (const int  *n, float *da, float *dx, const int  *incx, float *dy, const int  *incy);
int scopy_ (const int  *n, float *dx, const int  *incx, float *dy, const int  *incy);
float sdot_ (const int  *n, float *dx, const int  *incx, float *dy, const int  *incy);
float snrm2(const int  *n, float *dx, const int  *incx);
int sscal_ (const int  *n, float * da, float *dx, const int  *incx);
int sswap_ (const int  *n, float *dx, const int  *incx, float *dy, const int  *incy);
int srot_ (const int  *n, float *dx, const int  *incx, float *dy, const int  *incy, float  *c, float  *s);
#define BLAS_H
#endif
...
#else
  if(SINGLE_REAL){
    sum=sdot_(&N,(float *)xd,&one,(float *)yd,&one);
  }else{
    sum=ddot_(&N,(double *)xd,&one,(double *)yd,&one);
  }
#endif
...
#else
  if(SINGLE_REAL){
    scopy_(&N,(float *)xd,&one,(float *)zd,&one);
  }else{
    dcopy_(&N,(double *)xd,&one,(double *)zd,&one);
  }
#endif
...
#else
  if(SINGLE_REAL){
    saxpy_(&N,(float *)&a,(float *)xd,&one,(float *)yd,&one);
  }else{
    daxpy_(&N,(double *)&a,(double *)xd,&one,(double *)yd,&one);
  }
#endif
  return;
...
#else
  if(SINGLE_REAL){
    sscal_(&N,(float *)&a,(float *)xd,&one);
  }else{
    dscal_(&N,(double *)&a,(double *)xd,&one);
  }
#endif
  return;
}
