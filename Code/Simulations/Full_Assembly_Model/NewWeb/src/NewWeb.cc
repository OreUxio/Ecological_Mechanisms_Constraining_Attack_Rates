// $Id: NewWeb.cc 2473 2016-06-10 15:15:58Z axel $
#include <string.h>
#include <stdlib.h>
...
int NewWeb::delete_species_and_report(int i){
	...
	list_of_inserted_deleted_species << s(i).unique_id() << " " << s(i).parent_unique_id() << " " << 0 << " " << s(i).aggressivity_g()  << " " << 0 << " " << 0 << " " << 0 << " " << 0 << endl;
...
//     delete_species_and_report(new_species);			\
//     s=save_s;						\
//   }while(true);						\
//   if(attempts_left <=0){				\
...
void NewWeb::enforce_equilibrium(){
  ...
  do{
    ...
    if(bad_species>=0){
      ...
      delete_species_and_report(bad_species);
    }else{
      return;
    }
  }while(true);
...
double NewWeb::fitness(NewSpecies & test_species){
  ...
  delete_species_and_report(test);
  return f;
}

void NewWeb::delete_random_animal_species(){
  delete_species_and_report(unirand()*number_of_animals());
}
