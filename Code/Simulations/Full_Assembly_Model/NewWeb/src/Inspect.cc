// $id: Inspect.cc 2064 2011-01-26 17:30:30Z axel $
#include <string>
std::string version("$Revision: 2475 $");
...
-6 <filename>  Simulate invasion of species defined by aggressiveness is stored in <filename>\n\
...
void read_arguments(int argc,char *argv[]){
  ...
  const char * formatstring= 
    "k:K:Q:N:M:n:ma::B:E:L:q:G:V:P:T:u:U:D:ihvg::sxbfdH:ct:plr:Z:S:12yYA:C:R:X:w:W:I::jF:e:OoJz::3:05:4:6";
...
//~ #endif // experimental code to make movies
	...
	//~ cout << "finished" << endl;
		...
		//~ NewSpecies * new_species = web.speciate_only_fit(NewSpecies::plant);
		...
		//~ while(invaded==0){
			//~ attempt = attempt+1;
			//~ NewSpecies * new_species = web.speciate_only_fit(NewSpecies::plant);
			//~ fitness = web.invasion_fitness(new_species);
			//~ if(fitness>0){
				//~ invaded = 1;
			//~ }
			//~ if(attempt>1000){
			...
			//~ }
		//~ }
		//~ string out1 = "list_of_inserted_deleted_biomass_species_w.dat";
		//~ string out2 = "list_of_inserted_deleted_biomass_species_ww.dat";
		//~ string out3 = "list_of_inserted_deleted_biomass_species_www.dat";
		//~ if(invaded==1){
			//~ list_of_inserted_deleted_biomass_species_w.open(out1.c_str(),ios_base::app);
			//~ list_of_inserted_deleted_biomass_species_ww.open(out2.c_str(),ios_base::app);
			//~ list_of_inserted_deleted_biomass_species_www.open(out3.c_str(),ios_base::app);
			//~ int jj=web.s.size();
			//~ for(int i=jj;i-->0;){
				//~ list_of_inserted_deleted_biomass_species_w << fitness << " " << web.s(i).aggressivity_g()  << " " << web.s(i).biomass_abundance_B()  << " " << jj << " " << web.s(i).unique_id() << " " << number_of_next_save << " " << random_seed << endl;
			//~ }
			//~ web.insert(new_species);
			//~ jj=web.s.size();
			//~ for(int i=jj;i-->0;){
				//~ list_of_inserted_deleted_biomass_species_ww << fitness << " " << web.s(i).aggressivity_g()  << " " << web.s(i).biomass_abundance_B()  << " " << jj << " " << web.s(i).unique_id() << " " << number_of_next_save << " " << random_seed << endl;
			//~ }
			//~ int time_between_insertions=1000;
			//~ web.relax(time_between_insertions,NewWeb::species_set_t(),true);
			//~ jj=web.s.size();
			//~ for(int i=jj;i-->0;){
				//~ list_of_inserted_deleted_biomass_species_www <<  fitness << " " << web.s(i).aggressivity_g()  << " " << web.s(i).biomass_abundance_B()  << " " << jj << " " << web.s(i).unique_id() << " " << number_of_next_save << " " << random_seed << endl;
			//~ }
		//~ }
      }
	...
	XMLStore()
	  ...
	  if(aggressivenes_file){
		...
		stringstream geek(aggressivenes_file);
		double agg = 0; geek >> agg;
		...
		//~ if(invaded==1){
			...
			//~ int jj=web.s.size();
			//~ for(int i=jj;i-->0;){
				//~ list_of_inserted_deleted_biomass_species_ww << agg << " " << fitness << " " << web.s(i).aggressivity_g()  << " " << web.s(i).biomass_abundance_B()  << " " << jj << " " << web.s(i).unique_id() << " " << number_of_next_save << " " << random_seed << endl;
			//~ }
			//~ int time_between_insertions=1000;
			//~ web.relax(time_between_insertions,NewWeb::species_set_t(),true);
			//~ jj=web.s.size();
			//~ for(int i=jj;i-->0;){
				//~ list_of_inserted_deleted_biomass_species_www << agg << " " << fitness << " " << web.s(i).aggressivity_g()  << " " << web.s(i).biomass_abundance_B()  << " " << jj << " " << web.s(i).unique_id() << " " << number_of_next_save << " " << random_seed << endl;
			//~ }
		//~ }
      }
