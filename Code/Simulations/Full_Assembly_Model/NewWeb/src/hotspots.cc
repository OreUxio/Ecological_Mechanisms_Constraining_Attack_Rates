// $Id: hotspots.cc 2479 2016-10-23 11:42:18Z axel $
...
#define ONLY_FIT_DEFINE(X)					\
  NewSpecies * NewWeb::X##_only_fit(double plant_fraction){	\
    return find_only_fit(plant_fraction,&NewWeb::X);		\
  }

ONLY_FIT_DEFINE(invade);
ONLY_FIT_DEFINE(speciate);
ONLY_FIT_DEFINE(speciate_selected);
ONLY_FIT_DEFINE(invade_or_speciate);
ONLY_FIT_DEFINE(invade_or_speciate_any);
ONLY_FIT_DEFINE(invade_or_speciate_any_by_biomass);
#undef ONLY_FIT_DEFINE
