  ...
  if(fast_stats and file_type!=xml){
	...
	//~ }
	  ...
	  if(aggressivenes_file){
		...
		list_of_inserted_deleted_biomass_species_ww.open("before.dat",ios_base::app);
		list_of_inserted_deleted_biomass_species_www.open("after.dat",ios_base::app);
		NewSpecies * new_species = web.speciate_only_fit(NewSpecies::animal);
		web.initialize_for_integration();
		double fitness = web.invasion_fitness(new_species);
		int jj=web.s.size();
		for(int i=jj;i-->0;){
			list_of_inserted_deleted_biomass_species_ww << agg << " " << web.s(i).aggressivity_g()  << " " << web.s(i).biomass_abundance_B()  << " " << jj << " " << web.s(i).unique_id() << " " << number_of_next_save << " " << random_seed << endl;
		}
		web.insert(new_species);
		for(int i=jj;i-->0;){
